...
public class Scheduler {
     ...
     * @author: Quackster
     */
    public static <T> T executeRegionCall(Plugin plugin, Callable<T> task, Location location) {
        try {
            if (FOLIA) {
                    ...
                    return CompletableFuture
                            .supplyAsync(() -> {
                                try {
                                    ...
                                    plugin.getLogger().log(Level.SEVERE, "Error running ", ex);
                                }
                                return null;
                            }, command -> Bukkit.getRegionScheduler().execute(plugin, location, command))
                            .join();
                }
            }
     ...
     * @author: Quackster
     */
    public static <T> T executeRegionCall(Plugin plugin, Callable<T> task, Entity entity) {
        try {
            if (FOLIA) {
                if (!Bukkit.isOwnedByCurrentRegion(entity)) {
                    return CompletableFuture
                            .supplyAsync(() -> {
                                ...
                                return null;
                            }, command -> entity.getScheduler().execute(plugin, command, null, 0L))
                            .join();
                }
            }
